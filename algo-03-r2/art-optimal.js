/*
root@raspberrypi:~/projet-smb215# node algo-03-r2/art-optimal.js
root@raspberrypi:~/projet-smb215# perf stat node algo-03-r2/art-optimal.js
*/
...
fs.readFile(INPUT_FILE, 'utf8', (err, data) => {
    ...
    const findMostEfficientElem = (x, y, xi, yj, found) => {
        const elem = drawableArea[getArrayIndex(xi, yj)];
        if (elem && isInArea(x, y, elem)) {
            const weight = computeWeightArea(elem);
            if (weight === 0) {
                drawableArea[getArrayIndex(xi, yj)] = undefined;
    ...
    const fillHeaviestArea = (x, y) => {
        // Check x, y
        let found = findMostEfficientElem(x, y, x, y, [{x: 0, y: 0, d: 0, w: 0}, 0]);
        let d = 1;
        let yMax = (y - maxSize > 0) ? y - maxSize : 0;
        let xMax = (x - maxSize > 0) ? x - maxSize : 0;
        while (d <= maxSize) {
            let ySearch = ((y - d) > 0) ? (y - d) : 0;
            let xSearch = ((x - d) > 0) ? (x - d) : 0;
            for (let i = 0; i < d; i++) {
                let yj = y - i;
                let xi = x - i;
                // check in line ySearch
                if (xMax < (xi)) {
                    if (baseGrid[ySearch][xi] !== "*") {
                        found = findMostEfficientElem(x, y, xi, ySearch, found);
                    } else {
                        xMax = Math.max(xMax, xi);
                        yMax = Math.max(yMax, ySearch);
                    }
                }
                // check in row xSearch
                if (yMax < (yj)) {
                    if (baseGrid[yj][xSearch] !== "*") {
                        found = findMostEfficientElem(x, y, xSearch, yj, found);
                    } else {
                        yMax = Math.max(yMax, yj);
                        xMax = Math.max(xMax, xSearch);
                    }
                }
            }
            // check angle
            if (xMax < xSearch && yMax < ySearch) {
                if (baseGrid[ySearch][xSearch] !== "*") {
                    found = findMostEfficientElem(x, y, xSearch, ySearch, found);
                } else {
                    xMax = Math.max(xMax, xSearch);
                    yMax = Math.max(yMax, ySearch);
                }
