/*
root@raspberrypi:~/projet-smb215# gcc -w -Wall algo-03-r1/art-optimal.c -o algo-03-r1/art-optimal.o
root@raspberrypi:~/projet-smb215# chmod +x algo-03-r1/art-optimal.o
root@raspberrypi:~/projet-smb215# perf stat ./algo-03-r1/art-optimal.o
*/
...
#define INPUT_FILE  "./algo-03-r1/input.txt"
#define OUTPUT_FILE "./algo-03-r1/solution.txt"
...
static drawableElem **drawableIndex;
    ...
    for (int y = 0; y < baseY; y++) {
        for (int x = 0; x < baseX; x++) {
            if (getBaseGrid(x, y) == HASH) {
                ...
                drawableIndex[getArrayIndex(x, y)] = elem;
            }
        }
    }
    return head;
...
drawableElem *fillHeaviestArea(int x, int y) {
    ...
    drawableElem *elem = NULL;
    int yMin = (y - maxSize > 0) ? y - maxSize : 0;
    int xMin = (x - maxSize > 0) ? x - maxSize : 0;
    for (int j = yMin; j <= y; j++) {
        for (int i = xMin; i <= x; i++) {
            if (getBaseGrid(i, j) != STAR) {
                elem = drawableIndex[getArrayIndex(i, j)];
                if (elem->d != 0 && isInArea(x, y, *elem)) {
...
int main() {
    ...
    drawableIndex = malloc(sizeof(drawableElem *) * gridLen);
    memset(drawableIndex, 0, sizeof(drawableElem *) * gridLen);
    ...
    for (int index = 8; index > 1; index--) {
        ...
        while (elem->next != NULL) {
            if (getBaseGrid(elem->x, elem->y) == HASH && index == elem->i) {
